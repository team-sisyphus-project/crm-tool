-- Add the associated campaign to contacts and companies.
-- Nullable: every existing row has no campaign yet.
alter table public.companies add column if not exists campaign text;
alter table public.contacts add column if not exists campaign text;
